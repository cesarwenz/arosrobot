...
void initDrive()
{
  ...
  talFRght.SetInverted(true);
  talBRght.SetInverted(true);
}

void drive(double fwd, double turn)
{
  double left = (fwd - turn)*0.2;
  double rght = (fwd + turn)*0.2; /* positive turn means turn robot LEFT */

  talFLeft.Set(ControlMode::PercentOutput, left);
  talBLeft.Set(ControlMode::PercentOutput, left);
  talFRght.Set(ControlMode::PercentOutput, rght);
  talBRght.Set(ControlMode::PercentOutput, rght);
  std::cout << "Left: " << left << " Right: " << rght << "\n";
}
